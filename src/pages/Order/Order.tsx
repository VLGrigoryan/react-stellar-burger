...
const Order: FC<OrderProps> = ({ order, ingredients, modal, huge, isUser }) => {
  const displayedIngredients = modal ? ingredients : ingredients.slice(0, 6);
  const getStatusAndColor = () => {
    if (!order) return { status: null, color: undefined };
    const status =
      order.status === "done"
        ? "Выполнен"
  ...
  return (
    <div
      className={`${huge} ${modal ? OStyles.modalOrderContainer : OStyles.orderContainer
        } `}
    >
      <p
        className={`${modal ? OStyles.modalOrderNumber : OStyles.orderNumber
          } text text_type_digits-default`}
      >
        #{order.number}
      </p>
      <FormattedDate
        className={`${modal ? OStyles.modalOrderDate : OStyles.orderData
          } text text_type_main-default text_color_inactive`}
        date={new Date(order.updatedAt)}
      />
      <p
        className={`${modal ? OStyles.modalOrderTitle : OStyles.orderTitle
      ...
      <ul
        className={`${modal ? OStyles.modalIngredientsList : OStyles.ingredientsList
          }`}
      >
